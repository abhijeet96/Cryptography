...
int isValidBlock(struct block * toCheck,struct block * genesis)
{
	...
	for(int i=0;i<splitTransactions.size();i++)
	{
		...
		if(!validTransaction(genesis,splitTransactions[i],validTillNow))
		{
			flag=0;
			break;
		}
...
float initial_balance;
	...
	if(pwd==pwd2)
	{
		...
		matched_vec[0].pb(mp(mp(1,initial_balance),mp("0",uid)));
		...
		{
			matched_vec[0].pb(mp(mp(stoi(portfolio_stocks[i]),0),mp(uid,"0")));
		}
	}
	else
	{

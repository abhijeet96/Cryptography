...
int validTransaction(struct block * genesis,string transaction,string validTillNow)
{
	...
	*/
	...
	struct block * last=getLatestBlock(genesis); //last added 

	if(buyerstr=="BANK"||sellerstr=="BANK")
	return 1;

	vector<string> validTillNowvec=split(validTillNow,':');	

	for(int i=0;i<validTillNowvec.size();i++)
	{
		vector<string> qpbs2=split(validTillNowvec[i],' ');
	...
	{
		//cout<<"OMG"<<endl;
		cout<<buyercash<<' '<<qty*price<<endl;
		cout<<sellerstocks<<' '<<qty<<endl;
		return 0;
	}
...
int isValidBlock(struct block * toCheck,struct block * genesis)
{
	...
	for(int i=0;i<splitTransactions.size();i++)
	{
		...
		if(validTransaction(genesis,splitTransactions[i],validTillNow)==0)
		{
			flag=0;
			break;
		}
		...
		{
			validTillNow+=(splitTransactions[i]+":");
		}
	}
...
void printChain(struct block * genesisNode)
{
	while(genesisNode->next!=NULL)
	{
		...
		cout<<genesisNode->transactions<<endl;
		cout<<genesisNode->prevhash<<endl<<endl;
	}
}
...
float initial_balance;
	...
	if(pwd==pwd2)
	{
		...
		matched_vec[0].pb(mp(mp(1,initial_balance),mp("BANK",uid)));

		cout<<"Enter initial portfolio: (sep-.)"<<endl;
		cin>>portfolio;

		vector<string> portfolio_stocks=split(portfolio,'.');

		for(int i=0;i<portfolio_stocks.size();i+=2)
		{
			matched_vec[stoi(portfolio_stocks[i])].pb(mp(mp(stoi(portfolio_stocks[i+1]),0),mp(uid,"BANK")));
		}
	}
	else
	{
...
void printOrderBook()
{
	cout<<"BUY orders till now:"<<endl<<endl;
		for(int i=0;i<15;i++)
		{
			for(int j=0;j<buy_vec[i].size();j++)
				{
	...
	cout<<"SELL orders till now:"<<endl<<endl;
		for(int i=0;i<15;i++)
		{
